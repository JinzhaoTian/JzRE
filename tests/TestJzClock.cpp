...
protected:
    void SetUp() override
    {
        clock = std::make_unique<JzRE::JzClock>();
    }
    ...
    std::unique_ptr<JzRE::JzClock> clock;
};

// 测试时钟初始状态
TEST_F(TestJzClock, InitialState)
{
    // 初始状态下，时间应该为0或接近0
...
// 测试时钟更新
TEST_F(TestJzClock, Update)
{
    // 第一次更新
    clock->Update();
    JzRE::F32 firstTime  = clock->GetTimeSinceStart();
    JzRE::F32 firstDelta = clock->GetDeltaTime();
    ...
    JzRE::F32 secondTime  = clock->GetTimeSinceStart();
    JzRE::F32 secondDelta = clock->GetDeltaTime();
...
TEST_F(TestJzClock, TimeScale)
{
    ...
    JzRE::F32 scaledDelta   = clock->GetDeltaTime();
    JzRE::F32 unscaledDelta = clock->GetDeltaTimeUnScale();
...
// 测试时间缩放系数
TEST_F(TestJzClock, ScaleCoefficient)
{
    clock->SetTimeScale(1.0f);
    JzRE::F32 originalScale = clock->GetTimeScale();
...
TEST_F(TestJzClock, FrameRate)
{
    ...
    JzRE::F32 frameRate = clock->GetFrameRate();
...
// 测试零时间缩放
TEST_F(TestJzClock, ZeroTimeScale)
{
...
// 测试负时间缩放
TEST_F(TestJzClock, NegativeTimeScale)
{
    clock->SetTimeScale(-1.0f);
...
// 测试大时间缩放值
TEST_F(TestJzClock, LargeTimeScale)
{
...
// 测试连续更新的时间累积
TEST_F(TestJzClock, TimeAccumulation)
{
    clock->Update();
    JzRE::F32 initialTime = clock->GetTimeSinceStart();

    JzRE::F32 totalExpectedTime = 0.0f;
    ...
    JzRE::F32 finalTime   = clock->GetTimeSinceStart();
