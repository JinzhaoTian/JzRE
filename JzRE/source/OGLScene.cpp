...
void OGLScene::AddModel(std::shared_ptr<JzModel> model)
{
    this->models.push_back(model);
}

void OGLScene::RemoveModel(std::shared_ptr<JzModel> model)
{
    this->models.erase(std::remove(models.begin(), models.end(), model), models.end());
}

std::vector<std::shared_ptr<JzModel>> OGLScene::GetModels() const
{
    return this->models;
}
